...
const navItems = [{ label: "Overview", Icon: LayoutDashboard }, { label: "Analyses", Icon: BarChart3 }, { label: "Recent", Icon: History }, { label: "Reports", Icon: FileText }, { label: "Alerts", Icon: Bell }];
    ...
    <div className="min-h-screen bg-[var(--ink-deep)] text-[var(--text-primary)]">
      <header className="sticky top-0 z-30 border-b border-[var(--line-dark)] bg-[var(--ink-deep)]/90 backdrop-blur-xl">
        <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2"><button onClick={() => setCommandOpen(true)} className="hidden items-center gap-2 rounded-lg border border-[var(--line-dark)] px-3 py-2 text-xs text-[var(--text-secondary)] hover:border-[var(--accent)] md:flex"><Command size={14} /> Search <kbd className="rounded bg-white/[.07] px-1.5 py-0.5 text-[10px]">⌘K</kbd></button><button aria-label="Toggle theme" onClick={() => setDark((value) => !value)} className="rounded-lg p-2 text-[var(--text-secondary)] hover:bg-white/[.06]">{dark ? <Sun size={18} /> : <Moon size={18} />}</button><button aria-label="Alerts" onClick={() => setActiveNav("Alerts")} className="relative rounded-lg p-2 text-[var(--text-secondary)] hover:bg-white/[.06]"><Bell size={18} />{alertCount ? <span className="absolute right-1 top-1 size-1.5 rounded-full bg-[var(--amber)]" /> : null}</button><button aria-label="Help" onClick={() => setHelpOpen(true)} className="rounded-lg p-2 text-[var(--text-secondary)] hover:bg-white/[.06]"><HelpCircle size={18} /></button><div className="grid size-8 place-items-center rounded-full border border-[var(--accent)]/30 bg-[var(--surface)] text-xs font-semibold text-[var(--accent)]">MR</div></div>
        </div>
      </header>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-4 border-t border-[var(--line-dark)] bg-[var(--surface)]/95 p-2 backdrop-blur-xl lg:hidden" aria-label="Mobile navigation">{[{ label: "Home", Icon: LayoutDashboard }, { label: "Recent", Icon: History }, { label: "New", Icon: Sparkles }, { label: "Alerts", Icon: Bell }].map(({ label, Icon }) => <button key={label} onClick={() => label === "New" ? resetAnalysis() : setActiveNav(label === "Home" ? "Overview" : label)} className="flex flex-col items-center gap-1 rounded-lg py-1.5 text-[10px] text-[var(--text-secondary)]"><Icon size={17} />{label}</button>)}</nav>
